...
rs-pdfは2種類のページタイプをサポートしています：

#### 静的ページ (static)

要素を相対位置または絶対位置に配置します。
...
自動ページ分割機能を持つページ形式です。  
共通要素は静的ページ同様に相対位置または絶対位置に配置します。  
コンテンツ要素がコンテンツ領域に収まらない場合は自動的に次ページに分割されます。
...
```json5
{
  ...
  "common": {
    "objects": [
      // 全ページに共通して表示される要素（ヘッダー、フッターなど）
    ]
  },
  "content": {
    "frame": {
      "point": { "x": 10.0, "y": 50.0 },
      "size": { "width": 190.0, "height": 240.0 }
    },
    "objects": [
      // 自動ページ分割される動的コンテンツ
    ]
...
また2ページ目以降に共通して表示される要素を変えることも可能です。
...
```json5
{
  ...
  "continuation": {
    "common_objects": [
      // 2ページ目以降に共通して表示される要素（ヘッダー、フッターなど）
    ],
    "content_frame": {
      // 2ページ目以降の動的コンテンツを表示する矩形
    }
  }
}
```
...
**動的ページの利点:**
- コンテンツ量が可変の文書（請求書、納品書など）に最適
- ヘッダーやフッターを全ページに自動的に配置
- コンテンツ領域を超える要素を自動的に次ページに配置
...
完全な設定例は以下のファイルを参照してください：

- 静的ページの例: `demo/static.json`
- 動的ページの例: `demo/dynamic.json`
